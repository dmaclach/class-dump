...
{
    CDSection *section = [[self.machOFile dataConstSegment] sectionWithName:@"__objc_classrefs"];

    CDMachOFileDataCursor *cursor = [[CDMachOFileDataCursor alloc] initWithSection:section];
    while ([cursor isAtEnd] == NO) {
        uint64_t val = [cursor readPtr];
        CDOCClass *aClass = [self classWithAddress:val];
        if (aClass != nil) {
            [self addClassReference:aClass];
        }
    }

    section = [[self.machOFile dataConstSegment] sectionWithName:@"__objc_superrefs"];
